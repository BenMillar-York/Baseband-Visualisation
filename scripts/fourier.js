...
const BRICK_WALL_THRESHOLD = 100;
...
function discreteFourierTransform(data) {
    ...
    for (let frequency = 0; frequency < numDataPoints; frequency++) {
        ...
        for (let n = 0; n < numDataPoints; n++) {
            ...
            let exponent = -2 * Math.PI * frequency * (n/numDataPoints);
...
function inverseFourierTransform(freqArray) {
    ...
    for (let k = 0; k < N; k++) {
        ...
        for (let n = 0; n < N; n++) {
            ...
            let exponent = new Complex(Math.cos(2 * Math.PI * k * (n/N)), Math.sin(2 * Math.PI * k * (n/N)));
...
function sample_then_dft(data) {
    let sampledData = sampleData(data, SAMPLES_PER_BIT);
    return discreteFourierTransform(sampledData);
}